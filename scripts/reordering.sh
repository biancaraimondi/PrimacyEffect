        ...
        if ! ( [ "$descending" == '1' ] && [ "$reorder" == 'False' ] ); then
            ...
            do
                ...
                do
                    ...
                    do
                        python src/reordering.py --model_name ${model} --tokenizer_name ${model} --reorder_labels ${reorder} --descending_order ${descending} --labels_query_order ${l_q_order} --similarity ${similarity} --dataset ${dataset}
                    done
                done
            done
        fi
