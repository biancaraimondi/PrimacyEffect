...
from utils import extractNum, cosine_similarity, euclidean_distance, manhattan_distance
...
def main(
    ...
    similarity: Optional[str] = "cosine_similarity",
    ...
    - similarity: the similarity metric to use, one of 'cosine_similarity', 'euclidean_distance', 'manhattan_distance'
    ...
    embedder = pipeline.model.get_input_embeddings()
    tokenizer = pipeline.tokenizer

    # Optimization: tokenize and embed the labels before the loop because they are always the same for each instance
    if reorder_labels:
        # Tokenize the labels
        label_tokens_list = [tokenizer(label, return_tensors="pt").input_ids for label in label_ls_initial]
        # Get embeddings for the labels
        label_embeds = [embedder(label_tokens.to('cuda')).squeeze(0) for label_tokens in label_tokens_list]
    ...
    for id in range(0, length):
        ...
        if reorder_labels:
            ...
            question_embeds = embedder(question_tokens.to('cuda')).squeeze(0)
            # Dictionary that contains, for each non-reordered label, the similarity with the question
            similarities = {}
            # Calculate the similarity between the question and each label
            for i, label_tokens in enumerate(label_tokens_list):
                token_similarities = []
                # Calculate the similarity between each token in the question and each token in the label
                for q_embed in question_embeds:
                    for l_embed in label_embeds[i]:
                        sim = None
                        if similarity == "cosine_similarity":
                            sim = cosine_similarity(q_embed, l_embed)
                        elif similarity == "euclidean_distance":
                            sim = -euclidean_distance(q_embed, l_embed)
                        elif similarity == "manhattan_distance":
                            sim = -manhattan_distance(q_embed, l_embed)
                        else:
                            raise ValueError("Similarity must be one of 'cosine_similarity', 'euclidean_distance', 'manhattan_distance', but got " + similarity)
                        token_similarities.append(sim)
                # Compute the mean of the similarities of each label_token-question_token pair
                mean_similarity = sum(token_similarities) / len(token_similarities)
                similarities[label_ls_initial[i]] = mean_similarity
